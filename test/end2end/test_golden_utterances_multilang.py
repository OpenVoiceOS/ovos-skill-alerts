...
def minicroft():
    ...
    _t0 = time.monotonic()
    # wait_for_trained=False: get_minicroft's own built-in wait blocks on
    # "mycroft.skills.trained", which only padatious ever emits. Padacioso
    # (this fixture's file-intent engine, see _PIPELINE above) has no
    # training phase and never fires it, so that built-in wait would just
    # run out the clock and raise. wait_for_minicroft_ready below is the
    # real completion check for this fixture's engine choice.
    mc = get_minicroft([SKILL_ID], secondary_langs=LANGS, max_wait=300,
                       default_pipeline=_PIPELINE, wait_for_trained=False)
